...
for (fmt in formats) {
  ...
  if (!travis && fmt == 'bookdown::epub_book')
    bookdown::calibre('_book/ISCGwR.epub', 'mobi')
}

# r = '<body onload="window.location = \'https://bookdown.org/xiangyun\'+location.pathname">'
...
if(!travis){
  system("rm ISCGwR.rds packages.bib")
  # if(formats == "bookdown::pdfbook") system("rm 03-plot-tikzDictionary")
}
